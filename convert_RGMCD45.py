...
#-----------------------------------------------------------------------
# Program written to convert the global MCD45A1 maps into coarser
#  resolution maps following the BA extent proportion 
# 
...
for lat02 in range(0, limlat):
	for lon02 in range(0, limlon):
		...
		BA_nP[lat02, lon02]=(step*step)-varDB[varDB>800].shape[0]
		BA_nD[lat02, lon02]=varDB[(varDB>0) & (varDB<400)].shape[0]
		if BA_nD[lat02, lon02]>0:
			BA_MD[lat02, lon02]=numpy.median(varDB[(varDB>0) & (varDB<400)])
			BA_mD[lat02, lon02]=numpy.min(varDB[(varDB>0) & (varDB<400)])
			BA_MU[lat02, lon02]=numpy.mean(varUN[(varDB>0) & (varDB<400)])
